...
export abstract class BaseElementManager<T> {
    ...
    constructor(options: BaseElementManagerOptions = {}) {
        this.options = {
            attribute: options.attribute || `data-ss-${this.getComponentName()}`,
    ...
    public async init(): Promise<void> {
        ...
        const elements = this.options.root.querySelectorAll<HTMLElement>(selector)
    ...
    protected async processElement(element: HTMLElement): Promise<T | null> {
        ...
        try {
            ...
            // Mark as initialized
            element.setAttribute(`${this.options.attribute}-initialized`, "true")

            return instance
        } catch (error) {
            console.error(`[Stylescape] Error initializing ${this.getComponentName()}:`, error)
            return null
        }
    }
     ...
     */
    protected parseConfig(element: HTMLElement): Record<string, any> {
        ...
        if (jsonConfig) {
            try {
                ...
                console.warn(`[Stylescape] Invalid JSON config for ${this.getComponentName()}`)
            }
        }
