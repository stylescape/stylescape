...
export class ResponsiveMenuManager {
    ...
    constructor(
        ...
        this.menu = typeof menuSelector === "string"
            ? document.querySelector<HTMLElement>(menuSelector)
            : menuSelector

        this.toggle = typeof toggleSelector === "string"
    ...
    public destroy(): void {
        ...
        this.menu?.querySelectorAll(this.options.linkSelector).forEach((link) => {
    ...
    public static initMenus(): ResponsiveMenuManager[] {
        ...
        document.querySelectorAll<HTMLElement>('[data-ss="responsive-menu"]').forEach((nav) => {
            const toggle = nav.querySelector<HTMLElement>("[data-ss-menu-toggle]")
            const menu = nav.querySelector<HTMLElement>("[data-ss-menu-content]")

            if (toggle && menu) {
                const breakpoint = nav.dataset.ssMenuBreakpoint

                managers.push(new ResponsiveMenuManager(menu, toggle, {
                    breakpoint: breakpoint ? parseInt(breakpoint, 10) : undefined
                }))
            }
        })
