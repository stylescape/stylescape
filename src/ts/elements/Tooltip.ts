...
export class Tooltip {
    ...
    constructor(
        ...
        this.triggerElement = typeof selectorOrElement === "string"
            ? document.querySelector<HTMLElement>(selectorOrElement)
            : selectorOrElement

        const triggers = options.trigger
            ? (Array.isArray(options.trigger) ? options.trigger : [options.trigger])
            : ["hover", "focus"]

        this.options = {
            content: options.content ?? this.triggerElement?.getAttribute("title") ?? "",
    ...
    public static initTooltips(): Tooltip[] {
        ...
        document.querySelectorAll<HTMLElement>('[data-ss="tooltip"]').forEach((el) => {
            const content = el.dataset.ssTooltipContent || el.getAttribute("title") || ""
            const position = el.dataset.ssTooltipPosition as TooltipPosition || "top"
            const trigger = el.dataset.ssTooltipTrigger?.split(",") as TooltipTrigger[] || ["hover", "focus"]
    ...
    private position(): void {
        ...
        switch (position) {
            case "top":
                top = triggerRect.top + scrollY - tooltipRect.height - this.options.offset
            ...
            case "left":
                ...
                left = triggerRect.left + scrollX - tooltipRect.width - this.options.offset
                break
            case "right":
                top = triggerRect.top + scrollY + (triggerRect.height - tooltipRect.height) / 2
    ...
    private calculateBestPosition(triggerRect: DOMRect, tooltipRect: DOMRect): TooltipPosition {
        const space = {
            top: triggerRect.top,
