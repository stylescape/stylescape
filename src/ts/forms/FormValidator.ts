...
export interface FormValidatorOptions {
    ...
    onFieldValidate?: (field: HTMLInputElement, isValid: boolean, errors: string[]) => void
}
...
export class FormValidator {
    ...
    constructor(
        ...
        this.form = typeof formSelectorOrElement === "string"
            ? document.querySelector<HTMLFormElement>(formSelectorOrElement)
    ...
    private getFields(): HTMLInputElement[] {
        if (!this.form) return []
        return Array.from(this.form.querySelectorAll<HTMLInputElement>(
    ...
    private parseDataRules(field: HTMLInputElement): ValidationRule[] {
        ...
        if (field.hasAttribute(`${prefix}required`)) {
            rules.push({
                type: "required",
                message: field.getAttribute(`${prefix}required`) || "This field is required"
            })
        }
        ...
        if (field.hasAttribute(`${prefix}email`)) {
            rules.push({
                type: "email",
                message: field.getAttribute(`${prefix}email`) || "Invalid email address"
            })
        }
        ...
        if (pattern) {
            rules.push({
                ...
                message: field.getAttribute(`${prefix}pattern-message`) || "Invalid format"
            })
        }
        ...
        if (match) {
            rules.push({
                type: "match",
                value: match,
                message: field.getAttribute(`${prefix}match-message`) || "Fields do not match"
            })
        }
    ...
    private checkRule(rule: ValidationRule, value: string, field: HTMLInputElement): boolean {
        switch (rule.type) {
            case "required":
            ...
            case "match":
                const matchField = this.form?.querySelector<HTMLInputElement>(`[name="${rule.value}"]`)
