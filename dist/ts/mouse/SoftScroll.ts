        ...
        links.forEach((link) => {
            ...
            link.addEventListener('click', (event: Event) => {
                ...
                if (targetElement) {
                    const yOffset =
                        yOffsetInRem *
                        parseFloat(getComputedStyle(document.documentElement).fontSize) // Convert rem offset to pixels
                    const topPosition =
                        targetElement.getBoundingClientRect().top + window.pageYOffset - yOffset // Calculate the scroll position
