...
describe("Accordion", () => {
    ...
    beforeEach(() => {
        ...
        accordionElement = document.getElementById("test-accordion") as HTMLElement
    })
    ...
    describe("Callbacks", () => {
        it("should call onExpand callback", async () => {
            ...
            accordion = new AccordionManager(accordionElement, { onExpand })

            const trigger = $<HTMLElement>("[data-ss-accordion-trigger]")
            if (trigger) {
                click(trigger)
                await wait(50)
                // onExpand should have been called
            }
        })

        it("should call onCollapse callback", async () => {
            const onCollapse = vi.fn()
            accordion = new AccordionManager(accordionElement, { onCollapse })
