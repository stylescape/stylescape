...
describe("FormValidator", () => {
    ...
    afterEach(() => {
        if (
            formValidator &&
            typeof (formValidator as any).destroy === "function"
        ) {
    ...
    describe("Callbacks", () => {
        it("should call onFieldValidate callback", async () => {
        ...
        it("should call onValid callback on valid submit", async () => {
            const onValid = vi.fn();
            formValidator = new FormValidator(formElement, { onValid });
        });
    });
});
